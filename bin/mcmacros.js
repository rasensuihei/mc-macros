...
function loadDatapack (settings) {
  for (const [key, value] of Object.entries(settings.datapack)) {
...
function writeDatapack (settings) {
  Object.values(settings.datapack).forEach(value => {
    const file = path.normalize(path.join(settings.datapackDir, value.path))
...
async function main () {
  ...
  const reducer = (acc, cur) => {
    for (const [file, currentBuilder] of Object.entries(cur)) {
